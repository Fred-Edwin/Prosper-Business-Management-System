...
// Role gate + query validation only. The report's arithmetic is covered
// by lib/domain/financials/get-cash-flow.test.ts.
...
const PREFIX = "__fin_cashflow_route_test__";

async function get(qs: string) {
  const { GET } = await import("./route");
  const res = await GET(
    new NextRequest(`http://test/api/financials/cash-flow${qs}`),
  );
  return { status: res.status, body: await res.json() };
}

describe("/api/financials/cash-flow role gate", () => {
  let adminId: string;
  ...
  it("200 for an admin with a valid range", async () => {
    ...
    expect(res.body.data).toHaveProperty("openingBalances");
    expect(res.body.data).toHaveProperty("closingBalances");
    expect(res.body.data.openingBalances).toHaveProperty("cash");
    expect(res.body.data.openingBalances).toHaveProperty("mpesaBank");
  });
});
