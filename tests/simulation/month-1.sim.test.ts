// ═══════════════════════════════════════════════════════════════════════
// Horizon 2 — ONE MONTH of continuous trading, through the real API.
//
...
const DAYS = Array.from({ length: 31 }, (_, i) => addBusinessDays(START, i));
...
describe("one month of continuous trading", () => {
  beforeAll(async () => {
    vi.useFakeTimers();
    await resetLedger();
    cast = await loadCast();
  ...
  it("I4 — the month's revenue matches the shadow ledger exactly", async () => {
  ...
  it("I1 — the sum of the 31 daily summaries equals the month's summary", async () => {
    let revenue = 0n;
    let cogs = 0n;
    let expenses = 0n;
  ...
  it("I2 — COGS telescopes across every split point of the month", async () => {
  ...
  it("I2b — revenue telescopes across every split point of the month", async () => {
  ...
  it("I7 — debts owed to the business match the shadow at month end", async () => {
    const s = await summary(START, LAST);
