...
describe("/admin/financials — Stock Purchases tab", () => {
  ...
  it("a voided payment shows a Voided status even though it's not in awaitingReceipt (server-folded voided:true wins)", async () => {
    api.listMovements.mockImplementation(
      ({ movementType }: { movementType: string }) =>
        Promise.resolve(
          movementType === "purchase_payment"
            ? [
                movement({
                  id: "pp-voided",
                  purchaseSupplier: "Farmer's Choice",
                  purchaseTotalCost: "0.00",
                  voided: true,
                }),
              ]
            : [],
...
describe("/admin/financials — Deliveries tab", () => {
  ...
  it("a voided delivery shows a Voided status and no Void action", async () => {
    api.listMovements.mockImplementation(
      ({ movementType }: { movementType: string }) =>
        Promise.resolve(
          movementType === "purchase_receipt"
    ...
    const table = screen.getByRole("table");
    expect(within(table).getByText("Voided")).toBeInTheDocument();
    expect(within(table).queryByRole("button", { name: "Void" })).not.toBeInTheDocument();
  });
});
