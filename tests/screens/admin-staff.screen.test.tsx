...
describe("Pay — record a bonus (ADR-79)", () => {
    ...
    render(
      <ToastProvider placement="top-right">
        <PayTab
          month="2026-09"
          today={todayStr}
          registerRecordAdjustment={(fn) => (openAdvance = fn)}
        />
      </ToastProvider>,
    );
  }

  let openAdvance: () => void = () => {};
  ...
  it("records a bonus through the advance/deduction/bonus drawer", async () => {
    const user = userEvent.setup();
    ...
    await user.click(await screen.findByRole("option", { name: /Grace Wanjiru/ }));
    await user.click(within(dialog).getByRole("radio", { name: "Bonus" }));
    await user.type(within(dialog).getByLabelText(/^Amount/), "500");
    await user.click(within(dialog).getByRole("button", { name: "Save" }));

    await waitFor(() => expect(recordAdjustment).toHaveBeenCalledOnce());
    expect(recordAdjustment).toHaveBeenCalledWith(
      expect.objectContaining({ staffId: "s1", type: "bonus", amount: "500" }),
    );
    expect(await screen.findByText("Bonus recorded")).toBeInTheDocument();
  });

  it("shows a non-zero Bonuses cell as a button that opens the adjustments drawer", async () => {
    const user = userEvent.setup();
    payrollState = {
      payroll: payroll([pay({ bonuses: "500.00", adjustments: [adj({ type: "bonus", amount: "500.00" })] })]),
      loading: false,
      error: null,
    };
    renderPay();
    ...
    await user.click(
      screen.getByRole("button", { name: /Review bonuses for Grace Wanjiru/i }),
    );
    expect(
      await screen.findByRole("dialog", { name: /Advances, deductions & bonuses/i }),
    ).toBeInTheDocument();
  });
});
