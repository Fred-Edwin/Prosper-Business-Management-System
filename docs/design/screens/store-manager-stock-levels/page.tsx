// Screen skeleton transcribed from the Paper artboard "Store Manager — Stock Levels"
// (986-0) via get_jsx (Tailwind format). This is the screen *content* that renders inside
// the staff shell — the mobile status-bar, staff-shell header and bottom nav are dropped.
// Classes left as Paper emitted them; only literal data lifted into ./fixtures.ts.
//
// NO kit swap — the artboard has no PillFilter (contrary to the handoff's guess); it is a
// title block + a bespoke search row + a bespoke read-only list. The table header uses a
// `bg-info-bg` / `text-info` `text-[10px]` treatment with a `border-b-gray-600` rule, and
// rows are `h-[52px]` with a `--border-subtle` hairline (last row omits it). None of this
// maps onto a kit component drawn for it — kept verbatim (4b admin-stock-mobile
// precedent). Shares its shape with canteen-stock-levels (9GW-0).
//
